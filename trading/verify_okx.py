...
它用项目里的 OkxApi **真实代码路径**，对每个方向跑：
    换张数 → 证明空仓/清挂单 → 开仓 → 挂止损 → 查算法单 → 平仓 → 撤止损
逐项人工核对：
...
def acquire_verification_lock():
    """验证脚本与正式交易进程互斥，避免并发开平仓或互相撤单。"""
    from main import acquire_runner_lock
    return acquire_runner_lock()


def load_cfg():
    env_leverage = os.environ.get('OKX_LEVERAGE')
    cfg = {
        'apiKey': os.environ.get('OKX_API_KEY'),
        'secret': os.environ.get('OKX_API_SECRET'),
        'password': os.environ.get('OKX_API_PASSPHRASE') or os.environ.get('OKX_PASSWORD'),
        'margin_mode': os.environ.get('OKX_MARGIN_MODE'),
        'leverage': float(env_leverage) if env_leverage is not None else None,
    ...
    if os.path.exists(cfgfile):
        try:
            ...
            with open(cfgfile, 'r', encoding='utf-8') as f:
                raw = json.load(f)
            # 与 TradingSystem.load_config 同口径：当前顶层 okx 优先，旧 exchanges.okx 只作兜底。
            okx = raw.get('okx') or (raw.get('exchanges') or {}).get('okx') or {}
            for k in ('apiKey', 'secret', 'password', 'margin_mode', 'leverage', 'leverage_overrides'):
                if cfg.get(k) is None and okx.get(k) is not None:
                    cfg[k] = okx[k]
    ...
    if cfg.get('margin_mode') is None:
        cfg['margin_mode'] = 'cross'
    if cfg.get('leverage') is None:
        cfg['leverage'] = 3.0
    return cfg
...
def _prepare_verification_symbol(api, ccxt_symbol, label):
    """实弹前必须连续证明空仓并清空挂单；证据不足时绝不尝试开仓。"""
    try:
        if not api.confirm_position_flat(ccxt_symbol):
            print(f"[{label}] ❌ 无法连续证明交易所空仓，拒绝开始验证（不会触碰现有持仓/挂单）")
            return False
        if not api.cancel_all_orders(ccxt_symbol):
            print(f"[{label}] ❌ 验证前挂单清理无法确认，拒绝开始验证")
            return False
        return True
    except Exception as e:
        print(f"[{label}] ❌ 验证前置检查失败，拒绝开始验证: {e}")
        return False


def _cleanup_verification_attempt(api, ccxt_symbol, side, coin, label, stop_order_id=None):
    """开仓尝试后的统一清理：先平仓并连续证实空仓，再撤单，避免先撤保护。"""
    print(f"[{label}] --- 平仓 / 清理（保证执行）---")
    try:
        close_order = api.close_position(ccxt_symbol, side, coin)
        print(f"[{label}] 平仓返回:", (close_order or {}).get('id'))
        time.sleep(2)
        if not api.confirm_position_flat(ccxt_symbol):
            print(f"[{label}] ⚠️⚠️ 无法连续确认持仓归零；保留现有止损/挂单，请立即人工核对欧易！")
            return False

        specific_ok = True
        if stop_order_id:
            specific_ok = bool(api.cancel_order(ccxt_symbol, stop_order_id))
            print(f"[{label}] cancel_order:", specific_ok)
        cancel_all_ok = bool(api.cancel_all_orders(ccxt_symbol))
        if not cancel_all_ok:
            print(f"[{label}] ⚠️⚠️ 挂单清理无法确认，请立即人工核对欧易委托！")
        print(f"[{label}] ✓ 已连续确认空仓；挂单清理={'通过' if specific_ok and cancel_all_ok else '失败'}")
        return specific_ok and cancel_all_ok
    except Exception as e:
        print(f"[{label}] ❌ 清理失败，请立即手动到欧易检查持仓和委托！", e)
        return False
...
def run_side(api, ccxt_symbol, coin, side):
    ...
    if not _prepare_verification_symbol(api, ccxt_symbol, side):
        return False
    ...
    try:
        ...
        if not order:
            # 市价请求返回失败不等于交易所绝对未成交（可能迟到成交）；finally 仍须清理。
            print(f"[{side}] ❌ 开仓失败或成交状态不确定，立即进入清理")
            return False
        time.sleep(2)
        ...
        if entry <= 0:
            print(f"[{side}] ❌ 未取到真实入场价，无法验证止损链路，直接进入清理。")
            ok = False
        else:
        ...
        if not _cleanup_verification_attempt(
                api, ccxt_symbol, side, coin, side, stop_order_id):
            ok = False
    return ok


def _verify_fire_trigger(api, ccxt_symbol, coin, side, distance_pct, timeout_seconds, poll_interval):
    """执行实弹止损的证据检查；清理由外层统一完成后再裁决。"""
    time.sleep(2)
...
def run_fire_test(api, ccxt_symbol, coin, side, distance_pct=0.15, timeout_seconds=300, poll_interval=5):
    ...
    返回 True=触发且验证通过 / False=前置、验证或清理失败 / None=超时未触发
    ...
    label = f'fire-{side}'
    if not _prepare_verification_symbol(api, ccxt_symbol, label):
        return False
    ...
    try:
        order = api.open_position(ccxt_symbol, side, coin)
        print(f"[{label}] 开仓返回:", (order or {}).get('id'))
        if not order:
            print(f"[{label}] ❌ 开仓失败或成交状态不确定，立即进入清理")
        else:
            result = _verify_fire_trigger(
                api, ccxt_symbol, coin, side, distance_pct, timeout_seconds, poll_interval)
    finally:
        cleanup_ok = _cleanup_verification_attempt(
            api, ccxt_symbol, side, coin, label)
    if not cleanup_ok:
        return False
    return result
...
def main():
    ...
    try:
        # 局部变量持有文件对象直至 main() 返回，锁不会提前释放。
        _runner_lock = acquire_verification_lock()
    except RuntimeError as e:
        print(f"拒绝运行验证脚本：{e}。请先停止正式交易进程。")
        return 2
    ...
    if check_position_mode(api) is not True:
        print('❌ 无法证明账户为单向净持仓模式，拒绝继续验证或下单。')
        return 2

    bal = api.get_balance()
    usdt_total = (bal or {}).get('total', {}).get('USDT')
    try:
        usdt_total = float(usdt_total)
    except (TypeError, ValueError):
        usdt_total = float('nan')
    print("USDT 总额:", usdt_total)
    if not math.isfinite(usdt_total) or usdt_total < 0:
        print('❌ 无法取得有效 USDT 余额，验证失败。')
        return 2
    ...
    if args.fire:
        ...
        if result is True:
            ...
            print("⏱️ 本次仅因等待窗口内未自然触发而未获得证据；可调参数重试")
        return 0 if result is True else 2
