...
上层只使用内部符号（如 ``BTCUSDT``）和币数量；OKX 符号、合约张数、
算法止损及保证金细节全部封装在适配器内。
"""
...
class ExchangeApi:
    ...
    def confirm_position_flat(self, symbol):
        """多次查询确认交易所持仓已归零。"""
        raise NotImplementedError
    ...
    def ohlcv_to_dataframe(self, ohlcv):
        ...
        # CCXT 时间戳是 UTC epoch 毫秒。必须保留 UTC 时区；若转成 naive 后再
        # .timestamp()，Python 会按服务器本地时区解释，在 UTC+8 上把收盘时刻
        # 提前 8 小时，启动补跑可能误用尚未收盘的日线。
    ...
    def filter_closed_candles(self, df, timeframe='1d', grace_seconds=2):
        ...
        if last_open.tzinfo is None:
            # 兼容测试/旧调用方手工构造的 naive DataFrame；交易所时间一律按 UTC。
    ...
    def get_last_price(self, symbol):
        ...
        if not math.isfinite(price) or price <= 0:
            raise ValueError(f'{ccxt_symbol} 最新成交价不是正有限数: {ticker.get("last")!r}')
        return price
