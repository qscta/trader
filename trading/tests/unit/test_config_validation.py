...
class StrategyOhlcvLimitTest(unittest.TestCase):
    ...
    def test_unknown_strategy_rejected(self):
        # 仅 ma_cross 在白名单内；任何未知策略名一律 fail-loud（抛 ValueError）
        for strat in ('unknown', 'nonexistent'):
            with self.assertRaises(ValueError, msg=strat):
                cv.required_closed_candles_for_strategy(strat, {})
