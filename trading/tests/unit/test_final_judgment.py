"""终极审判：时间边界 / 真实并发混沌 / 灾难隔离 / 风控数学性质 / 孤儿仓告警。
...
class DisasterRecoveryTest(unittest.TestCase):
    ...
    主文件损坏 → 拒绝自动采用旧 .bak；主文件被删但 .bak 在 → 拒绝启动；
    主备都不存在 → 全新部署正常启动；人工仓/账本彻底丢失 → 孤儿仓告警兜底。"""

    def test_corrupted_main_refuses_stale_backup_auto_recovery(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(TradeStatePersistenceError, '禁止自动恢复'):
                TradeState(path)
            with open(path, encoding='utf-8') as f:
                self.assertEqual(f.read(), '{“损坏的JSON')
            with open(path + '.bak', encoding='utf-8') as f:
                self.assertIn('BTCUSDT', json.load(f)['open_positions'])
    ...
    def test_no_orphan_no_alert(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ensured = []
            system.notifier = SimpleNamespace(notify_error=lambda m: alerts.append(m) or True)
            system._ensure_stop_order_alive = lambda *args: ensured.append(args)
            system.exchange_api = SimpleNamespace(
                to_ccxt_symbol=lambda s: s,
                get_position=lambda s: {'contracts': 10.0},
                managed_position_matches=lambda *a, **k: True,
            ...
            self.assertEqual(ensured[0][0], 'BTCUSDT')
