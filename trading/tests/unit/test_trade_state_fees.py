...
class TradeStateFeeTest(unittest.TestCase):
    def test_nonfinite_exit_price_falls_back_without_corrupting_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            state = TradeState(os.path.join(tmp, 'trade_state.json'))
            state.add_open_position(
                'BTCUSDT', 'long', 100.0, 1.0, 80.0,
                'stop-1', strategy='ma_cross')

            closed = state.close_position('BTCUSDT', float('nan'))

            self.assertEqual(closed['exit_price'], 100.0)
            self.assertEqual(state.get_closed_trades()[0]['exit_price'], 100.0)
