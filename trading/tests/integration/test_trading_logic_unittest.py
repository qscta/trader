...
APP_DIR = Path(
    os.environ.get(
        "TRADING_SYSTEM_DIR",
        Path(__file__).resolve().parents[2],  # tests/integration 的上两级 = 仓库根
    )
)
...
class FilterClosedCandlesTests(unittest.TestCase):
    ...
    def test_drops_last_candle_when_not_closed(self):
        ...
        with patch(
            ...
            return_value=pd.Timestamp("2026-03-11 23:59:59", tz="UTC").timestamp(),
        ):
    ...
    def test_keeps_last_candle_when_closed_after_grace_window(self):
        ...
        with patch(
            "exchange_base.time.time",
            return_value=pd.Timestamp("2026-03-12 00:00:03", tz="UTC").timestamp(),
        ):
            filtered = self.api.filter_closed_candles(df, "1d")
        self.assertEqual(len(filtered), 2)

    def test_ccxt_epoch_conversion_keeps_explicit_utc(self):
        epoch_ms = 1773187200000
        df = self.api.ohlcv_to_dataframe(
            [[epoch_ms, 1, 2, 0.5, 1.5, 10]])
        self.assertEqual(str(df['timestamp'].dt.tz), 'UTC')
        self.assertEqual(
            int(df.iloc[0]['timestamp'].timestamp() * 1000), epoch_ms)


class LiveDailyHistoryValidationTests(unittest.TestCase):
    def setUp(self):
        self.strategy = main.MaCrossStrategy(2, 3, 2)  # required = 6
        self.now = pd.Timestamp('2026-03-12 00:00:05', tz='UTC').timestamp()

    @staticmethod
    def frame(dates, closes=None):
        timestamps = pd.to_datetime(dates, utc=True)
        return pd.DataFrame({
            'timestamp': timestamps,
            'close': closes if closes is not None else [100.0] * len(timestamps),
        })

    def test_continuous_latest_closed_history_passes(self):
        df = self.frame(pd.date_range('2026-03-06', '2026-03-11', freq='1D'))
        self.assertEqual(self.strategy.validate_live_history(df, self.now), (True, None))

    def test_stale_history_fails_closed(self):
        df = self.frame(pd.date_range('2026-03-05', '2026-03-10', freq='1D'))
        ok, reason = self.strategy.validate_live_history(df, self.now)
        self.assertFalse(ok)
    ...
    def test_gap_or_duplicate_fails_closed(self):
        ...
        ok, reason = self.strategy.validate_live_history(df, self.now)
        self.assertFalse(ok)
        self.assertIn('缺口', reason)

    def test_nonfinite_close_fails_closed(self):
        df = self.frame(
            pd.date_range('2026-03-06', '2026-03-11', freq='1D'),
            [100, 101, 102, float('nan'), 104, 105])
        ok, reason = self.strategy.validate_live_history(df, self.now)
        self.assertFalse(ok)
...
class InstantOpenApiTests(unittest.TestCase):
    ...
    def make_system(self, existing_position=None):
        ...
        # ma_cross 默认最少需 max(ma_long*2, ma_stop+1)=56 根已收盘K线，给足 60 根
        df_marker = [None] * 60
        ...
        system = SimpleNamespace(
            ...
            ma_cross_strategy=SimpleNamespace(
                validate_live_history=Mock(return_value=(True, None)),
    ...
    def test_instant_open_rejects_disabled_pool_symbol(self):
        self.authenticate()
    ...
    def test_instant_open_rejects_when_live_price_unavailable(self):
        self.authenticate()
        fake_system = self.make_system()
        fake_system.exchange_api.exchange.fetch_ticker.side_effect = RuntimeError('行情失败')
    ...
    def test_instant_open_uses_closed_candle_filter_and_executes_trade(self):
        ...
        self.assertEqual(stop_loss_price, 100)   # 多单止损取 lower_stop
    ...
    def test_instant_open_fetch_limit_tracks_large_ma_cross_config(self):
        self.authenticate()
        fake_system = self.make_system()
        fake_system.config = {
            "strategy": {"ma_long_period": 250, "ma_stop_period": 400},
            "trading": {"symbols": []},
        }
        df_marker = [None] * 500
        ...
        self.assertEqual(resp.status_code, 200)
        # required = max(250*2, 400+1) = 500；fetch_limit = max(365, 501) = 501
        fake_system.exchange_api.fetch_ohlcv.assert_called_once_with("BTC/USDT", "1d", limit=501)
    ...
    def test_instant_open_rejects_unknown_strategy(self):
        """仅 ma_cross 在白名单内：请求任何非白名单策略即 400 拒绝，不执行开仓。"""
        self.authenticate()
        fake_system = self.make_system()
        ...
        with patch.object(api_server, "trading_system", _prep_system(fake_system)), patch.object(
            ...
            resp = self.client.post(
                "/api/instant_open",
                json={"name": "BTCUSDT", "risk_per_trade": 0.01, "strategy": "nonexistent"},
            )
...
class SymbolInputValidationTests(unittest.TestCase):
    ...
    def test_add_symbol_rejected_while_trading_lock_held(self):
        self.system.config = {"trading": {"symbols": []}}
        self.system.exchange_api = SimpleNamespace(
            to_ccxt_symbol=_fake_to_ccxt,
            fetch_ohlcv=Mock(return_value=[[1, 2, 3, 4, 5, 6]]),
        )
        self.system._trade_lock = threading.Lock()
        self.system._trade_lock.acquire()
        try:
            resp = self._post(
                "/api/symbols", {"name": "BTCUSDT", "risk_per_trade": 0.01})
            self.assertEqual(resp.status_code, 409)
    ...
    def test_all_symbol_execution_changes_rejected_while_trading_lock_held(self):
        """禁用/风险度/策略都与交易执行互斥，防止 API 返回后日检仍按旧快照开仓。"""
        self.system.config = {"trading": {"symbols": [
            {"name": "BTCUSDT", "strategy": "ma_cross", "enabled": True}]}}
        ...
        try:
            with patch.object(api_server, "trading_system", self.system), patch.object(
                ...
                resp3 = self.client.put("/api/symbols/BTCUSDT", json={"risk_per_trade": 0.02})
                self.assertEqual(resp3.status_code, 409)
        finally:
            self.system._trade_lock.release()

    def test_strategy_params_rejected_while_trading_lock_held(self):
        self.system.config = {"strategy": {"ma_short_period": 7, "ma_long_period": 28}}
        self.system._trade_lock = threading.Lock()
        self.system._trade_lock.acquire()
        try:
            with patch.object(api_server, "trading_system", self.system), patch.object(
                ...
                resp = self.client.put(
                    "/api/strategy_params", json={"ma_short_period": 8})
            self.assertEqual(resp.status_code, 409)
            self.assertEqual(self.system.config["strategy"]["ma_short_period"], 7)
        finally:
            self.system._trade_lock.release()
    ...
    def test_strategy_params_rejects_fractional_period(self):
        ...
        self.system.config = {"strategy": {"ma_stop_period": 28}}
        with patch.object(api_server, "trading_system", self.system), patch.object(
            api_server, "send_dingtalk", Mock()
        ):
            resp = self.client.put("/api/strategy_params", json={"ma_stop_period": 28.9})
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(self.system.config["strategy"]["ma_stop_period"], 28)  # 未写入截断值
...
class DeleteSymbolApiTests(unittest.TestCase):
    """删除交易对：不立即平仓/撤单，已有仓转为只平不开，并清理重入标记。"""
    ...
    def make_system(self, held_position=None, config_strategy="ma_cross"):
        ...
        stop_loss_dates = {"ETHUSDT": "2000-01-01"}
        save_stop_loss_dates = Mock()

        def clear_stop_loss(symbol):
            if stop_loss_dates.pop(symbol, None) is None:
                return False
            save_stop_loss_dates()
            return True
        ...
        system = SimpleNamespace(
            ...
            stop_loss_dates=stop_loss_dates,
            _save_stop_loss_dates=save_stop_loss_dates,
    ...
    def test_delete_with_position_keeps_state_untouched(self):
        ...
        self.assertIn("只平不开", resp.get_json()["message"])
    ...
    def test_delete_rejected_when_strategy_unknowable(self):
        ...
        self.assertEqual(system.strategy_backfills, [])
        self.assertIn("ETHUSDT", system.stop_loss_dates)  # 删除未成功，标记不得误清

    def test_delete_rejected_while_trade_check_holds_lock(self):
        system = self.make_system(held_position={"symbol": "ETHUSDT", "strategy": "ma_cross"})
        _prep_system(system)
        system._trade_lock = SimpleNamespace(
            acquire=Mock(return_value=False), release=Mock())

        with patch.object(api_server, "trading_system", system), patch.object(
            api_server, "send_dingtalk", Mock()
        ):
            resp = self.client.delete("/api/symbols/ETHUSDT")

        self.assertEqual(resp.status_code, 409)
        self.assertEqual(len(system.config["trading"]["symbols"]), 1)
        system._trade_lock.release.assert_not_called()


class ManualCloseConsistencyApiTests(unittest.TestCase):
    """前端手动平仓不得用本地旧数量制造交易所残仓。"""
    ...
    def make_system(self, exchange_position):
        position = {
            "symbol": "BTCUSDT", "side": "long", "position_size": 1.0,
            "entry_price": 100.0, "stop_loss_price": 90.0, "stop_order_id": "stop-1",
        }
        system = SimpleNamespace(
            trade_state=SimpleNamespace(get_open_position=Mock(return_value=position)),
            exchange_api=SimpleNamespace(
                to_ccxt_symbol=_fake_to_ccxt,
                get_position=Mock(return_value=exchange_position),
                close_position=Mock(return_value={"average": 105.0}),
                get_last_price=Mock(return_value=105.0),
            ),
            _managed_position_is_consistent=Mock(return_value=True),
            _cancel_stop_order_confirmed=Mock(return_value=True),
            _close_trade_state_with_runtime_fallback=Mock(
                return_value=({"pnl": 5.0, "pnl_percent": 5.0}, True)),
            clear_stop_loss=Mock(return_value=False),
            label="欧易",
        )
        return _prep_system(system)

    def _close(self, system):
        with patch.object(api_server, "trading_system", system), patch.object(
            api_server, "send_dingtalk", Mock()
        ):
            return self.client.post("/api/close_position", json={"name": "BTCUSDT"})

    def test_mismatch_refuses_before_close(self):
        system = self.make_system({"contracts": 15, "side": "long"})
        system._managed_position_is_consistent.return_value = False
    ...
    def test_position_query_failure_refuses_before_close(self):
        system = self.make_system(None)
    ...
    def test_exact_match_closes_normally(self):
        ...
        self.assertEqual(resp.status_code, 200)
        system._managed_position_is_consistent.assert_called_once()
...
class ExecuteOpenRiskGuardTests(unittest.TestCase):
    def make_system(self):
        ...
        system.exchange_api = SimpleNamespace(
            ...
            confirm_position_flat=Mock(return_value=True),
    ...
    def test_deployment_gate_blocks_every_open_before_exchange_calls(self):
        system = self.make_system()
        system.new_entries_disabled = True
    ...
    def test_exit_only_or_disabled_config_blocks_every_open(self):
        for config in (
                {"name": "BTCUSDT", "enabled": False},
                {"name": "BTCUSDT", "exit_only": True}):
            with self.subTest(config=config):
                system = self.make_system()
                system._execute_open("BTCUSDT", "long", 100, 80, config)
                system.exchange_api.get_position.assert_not_called()
                system.exchange_api.open_position.assert_not_called()
    ...
    def test_timeout_confirmed_actual_size_drives_stop_and_ledger(self):
        system = self.make_system()
        system.exchange_api.open_position.return_value = {
            "id": "timeout_confirmed", "average": 100,
            "confirmed_coin_amount": 1.25,
        }
        ...
        system.trade_state.add_open_position.assert_called_once_with(
            "BTCUSDT", "long", 100, 1.25, 80, "stop-1", strategy="ma_cross")

    def test_invalid_confirmed_size_rolls_back_instead_of_forgetting_open(self):
        system = self.make_system()
        system.exchange_api.open_position.return_value = {
            "id": "timeout_confirmed", "average": 100,
            "confirmed_coin_amount": float('nan'),
        }
    ...
    def test_invalid_fill_price_rolls_back_instead_of_corrupting_state(self):
        system = self.make_system()
        system.exchange_api.open_position.return_value = {
            "id": "open-1", "average": float('inf')}
    ...
    def test_orphan_query_failure_blocks_open(self):
        """孤儿核对查询失败：无法证明空仓时 fail closed，防止叠加人工/未托管仓位。"""
        system = self.make_system()
        system.exchange_api.get_position.side_effect = RuntimeError("查询失败")
    ...
    def test_single_empty_response_cannot_authorize_preopen_cleanup(self):
        system = self.make_system()
        system.exchange_api.get_position.return_value = None
        system.exchange_api.confirm_position_flat.return_value = False
    ...
    def test_missing_current_balance_blocks_open_instead_of_using_stale_equity(self):
        system = self.make_system()
        system.exchange_api.get_balance.return_value = None
    ...
    def test_unverifiable_pending_order_cleanup_blocks_open(self):
        system = self.make_system()
        system.exchange_api.cancel_all_orders.return_value = False
    ...
    def test_rolls_back_when_stop_order_creation_fails(self):
        ...
        # 用同一父 mock 记录调用顺序：先确认平仓，再清扫疑似止损，
        # 避免平仓失败时把残仓唯一可能的保护先撤掉。
        calls = Mock()
        ...
        system.exchange_api.close_position.assert_called_once_with("BTC/USDT", "long", 2.5)
        # 开仓前清扫一次；止损创建失败时，只有平仓确认成功后才清扫疑似孤儿止损。
        self.assertEqual(system.exchange_api.cancel_all_orders.call_count, 2)
        system.exchange_api.cancel_all_orders.assert_called_with("BTC/USDT")
        self.assertEqual(
            [c[0] for c in calls.mock_calls],
            ["cancel_all_orders", "close_position", "cancel_all_orders"],
            "开仓前先清旧单；止损失败后先确认平仓，成功后再清本次疑似单",
        )
        system.trade_state.add_open_position.assert_not_called()
        system.notifier.notify_trade_opened.assert_not_called()

    def test_stop_failure_and_unconfirmed_rollback_keeps_position_managed(self):
        system = self.make_system()
        system.exchange_api.create_stop_loss_order.return_value = None
        ...
        system._execute_open(
            "BTCUSDT", "long", 100, 80,
            {"name": "BTCUSDT", "risk_per_trade": 0.01, "strategy": "ma_cross"},
        )

        system.trade_state.add_open_position.assert_called_once_with(
            "BTCUSDT", "long", 100, 2.5, 80, None, strategy="ma_cross")
        # 只发生开仓前清扫；回滚未确认时保留可能已到交易所的止损。
        self.assertEqual(system.exchange_api.cancel_all_orders.call_count, 1)

    def test_fill_cross_and_unconfirmed_rollback_keeps_position_managed(self):
        system = self.make_system()
        system.exchange_api.open_position.return_value = {"average": 79}
        system.exchange_api.close_position.return_value = None
        ...
        system.trade_state.add_open_position.assert_called_once_with(
            "BTCUSDT", "long", 79, 2.5, 80, None, strategy="ma_cross")

    def test_unconfirmed_stop_cleanup_persists_residue_before_rollback(self):
        system = self.make_system()
        system.exchange_api.create_stop_loss_order.return_value = None
        system.exchange_api.cancel_all_orders.side_effect = [True, False]
    ...
    def test_residue_persist_failure_cannot_interrupt_emergency_rollback(self):
        system = self.make_system()
        system.exchange_api.create_stop_loss_order.return_value = None
        system.exchange_api.cancel_all_orders.side_effect = [True, False]
        system.trade_state.mark_stop_residue.side_effect = trade_state.TradeStatePersistenceError(
            "磁盘故障")
        ...
        system.trade_state.force_runtime_mark_stop_residue.assert_called_once_with(
            "BTCUSDT")
        system.exchange_api.close_position.assert_called_once_with(
            "BTC/USDT", "long", 2.5)
        system.trade_state.add_open_position.assert_not_called()

    def test_residue_clear_persist_failure_cannot_interrupt_emergency_rollback(self):
        system = self.make_system()
        system.exchange_api.create_stop_loss_order.return_value = None
        system.exchange_api.cancel_all_orders.return_value = True
        system.trade_state.clear_stop_residue.side_effect = trade_state.TradeStatePersistenceError(
            "磁盘故障")
...
class MaCrossFlipTests(unittest.TestCase):
    ...
    def test_removed_symbol_reverse_signal_passes_exit_only_and_never_reopens(self):
        system = self.make_system()
        system.exchange_api.get_position = Mock(return_value={"contracts": 2, "side": "long"})
    ...
    def test_removed_symbol_external_flat_does_not_record_t1(self):
        ...
        system.stop_loss_dates = {"BTCUSDT": "2000-01-01"}
        system._save_stop_loss_dates = Mock()
        system._handle_exchange_flat_close = Mock(return_value=({"symbol": "BTCUSDT"}, True, True))
        position = {"side": "long", "position_size": 2.0, "stop_loss_price": 90.0}

        system.handle_open_position_ma_cross(
            "BTCUSDT", {"action": None, "current_close": 100.0}, position,
            {"name": "BTCUSDT", "strategy": "ma_cross", "exit_only": True}, df=object())
...
class TradeStateCallsiteCompensationTests(unittest.TestCase):
    ...
    def test_state_failure_and_unconfirmed_rollback_keeps_runtime_position(self):
        system = ExecuteOpenRiskGuardTests().make_system()
        system.trade_state.add_open_position.side_effect = (
        ...
        system.trade_state.force_runtime_add_open_position.assert_called_once_with(
            "BTCUSDT", "long", 100, 2.5, 80, "stop-1", strategy="ma_cross")
...
class SchedulerIntervalTests(unittest.TestCase):
    ...
    def test_scheduler_health_detects_live_and_stopped_thread(self):
        system = self._make()
        system.scheduler.start()
        try:
            self.assertTrue(system._scheduler_is_alive())
        finally:
            system.scheduler.shutdown()
        self.assertFalse(system._scheduler_is_alive())
