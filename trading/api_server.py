...
def add_symbol():
    ...
    try:
        ...
        clean, invalid = _validate_symbol_input(
            ...
            'ma_cross' if strategy_in is None else strategy_in,
        ...
        # 最终提交必须与日检互斥：否则品种先 append 到内存、写盘随后失败的
        # 窄窗里，日检可能拿到这个未提交品种并开仓，而 API 最终却返回添加失败。
        if not system._trade_lock.acquire(blocking=False):
            return jsonify({'error': '交易检查/巡检正在执行中，请稍后再添加交易对'}), 409
        try:
            with system._config_lock:
                ...
                err_resp = _commit_config_or_rollback(
                    system, 'trading', 'symbols', backup_symbols, '配置写入失败，交易对未添加')
                if err_resp:
                    return err_resp
        finally:
...
def update_symbol(symbol):
    ...
    try:
        ...
        symbol_u = clean['name']
        # enabled/risk/strategy 都会改变本轮是否开仓或开多大：必须与日检互斥，
        # 否则日检持有旧快照时，API 即使已返回「禁用成功」仍可能随后开仓。
        if not system._trade_lock.acquire(blocking=False):
            return jsonify({'error': '交易检查/巡检正在执行中，请稍后再更新交易对'}), 409
        try:
...
def delete_symbol(symbol):
    ...
    try:
        ...
        # 删除会把已有仓位切换为「只平不开」，必须与日检/巡检互斥，
        # 否则日检可能拿着删除前的配置快照在同一轮反手开新仓。
        if not system._trade_lock.acquire(blocking=False):
            return jsonify({'error': '交易检查/巡检正在执行，请稍后再删除'}), 409
        try:
            with system._config_lock:
                ...
                err_resp = _commit_config_or_rollback(
                    system, 'trading', 'symbols', backup_symbols, '配置写入失败，删除已回滚')
                if err_resp:
                    return err_resp
            system.clear_stop_loss(symbol_u)
        finally:
            system._trade_lock.release()

        suffix = '；当前持仓已转为只平不开，结束后彻底退出' if held else ''
        send_dingtalk(f'[{system.label}] 删除交易对: {symbol}{suffix}')
        return jsonify({'status': 'success', 'message': f'交易对 {symbol} 已删除{suffix}'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def update_strategy_params():
    ...
    try:
        ...
        if not system._trade_lock.acquire(blocking=False):
            return jsonify({'error': '交易检查/巡检正在执行中，请稍后再更新策略参数'}), 409
        try:
...
def instant_open():
    ...
    try:
        ...
        try:
            ...
            if pool_cfg and not pool_cfg.get('enabled', True):
                return jsonify({'error': f'{symbol_name} 已在品种池中禁用；禁用状态只管理已有仓位，禁止即时开仓'}), 409
            ...
            history_ok, history_error = system.ma_cross_strategy.validate_live_history(df)
            if not history_ok:
                return jsonify({'error': f'{symbol_name} K线历史不可用于真钱开仓: {history_error}'}), 503

            try:
                current_price = system.exchange_api.get_last_price(ccxt_symbol)
            except Exception as e:
                logger.error(f"{symbol_name} 获取实时市价失败，拒绝即时开仓: {e}")
                return jsonify({'error': f'{symbol_name} 无法取得实时市价，已拒绝即时开仓'}), 503
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return jsonify({'error': f'{symbol_name} 无法确认交易所实际持仓，已拒绝平仓: {e}'}), 503
            if exchange_position is not None and not system._managed_position_is_consistent(
...
def _run_trading_or_terminate_process():
    """交易主循环或调度线程不得静默死亡；退出即终止进程交给服务管理器重启。"""
    try:
        trading_system.start()
    except BaseException:
        logger.exception('交易主线程异常退出，终止当前进程')
    else:
        logger.critical('交易主线程意外返回，终止当前进程')
...
if __name__ == '__main__':
    ...
    _bootstrap()
    threading.Thread(target=_run_trading_or_terminate_process, daemon=True).start()
