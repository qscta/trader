...
- **分层测试矩阵**——纯标准库套件覆盖状态机、并发与灾难恢复；依赖套件用真实 pandas 锁定 EMA 数学及 Flask/ccxt 集成行为。
- **行情 fail-closed**——日检固定读取 OKX 最新单页 300 根，不为指标计算分页；最新已收盘日 K 陈旧、数据量不足或历史出现大跨度断层时，禁止该品种开仓、平仓与反手。
...
```
        TradingSystem (main.py)                装配 · 配置校验 · 状态护栏 · 日检总指挥 · 调度
          ...
          ├── SignalHandlersMixin  (signal_handlers.py)  信号处理：无仓开仓判定 / 有仓平仓翻转 / T+1 重入
